...
use Drupal\ek_finance\CashData;
...
class ReportController extends ControllerBase {
  ...
  public function reporting(Request $request) {
    ...
    if (isset($_SESSION['repfilter']['filter']) && $_SESSION['repfilter']['filter'] == 1) {
      ...
      if ($coid != 'all') {
        ...
        $excel = Url::fromRoute('ek_finance_reporting_excel', ['param' => $p], [])->toString();
        $items['excel'] = [
          '#markup' => "<a href='" . $excel . "' title='" . $this->t('Excel download') . "'><span class='ico excel green'/></a>",
        ];
        return [
  ...
  public function budgeting(Request $request) {
    ...
    if (isset($_SESSION['repfilter']['filter']) && $_SESSION['repfilter']['filter'] == 1) {
      ...
      $excel = Url::fromRoute('ek_finance_budgeting_excel', ['param' => $param], [])->toString();
      $items['excel'] = [
    ...
    return [
      ...
      '#items' => $items,
      '#attached' => [
        'library' => ['ek_finance/ek_finance.budgeting'],
      ],
    ];
  }
  ...
  public function balancesheet(Request $request) {
    ...
    if (isset($_SESSION['bsfilter']['filter']) && $_SESSION['bsfilter']['filter'] == 1) {
      ...
      $pdf = Url::fromRoute('ek_finance_extract.balance_sheet_pdf', ['param' => $param], [])->toString();
      $items['pdf'] = [
        '#markup' => "<a href='" . $pdf . "' title='" . $this->t('Export to pdf') . "' target='_blank'><span class='ico pdf red'/></a>",
      ];
  ...
  public function cashflow() {
    ...
    if (isset($_SESSION['cashflowfilter']['filter']) && $_SESSION['cashflowfilter']['filter'] == 1) {
      ...
      $cashData = new CashData($coid, $amortization, $baseCurrency, $rounding, $chart);
      $items = array_merge($items, $cashData->getData());
