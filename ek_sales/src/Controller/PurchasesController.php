...
class PurchasesController extends ControllerBase {
  ...
  public function ListPurchases(Request $request) {
    ...
    if (isset($_SESSION['pfilter']) && $_SESSION['pfilter']['filter'] == 1) {
      if ($_SESSION['pfilter']['keyword'] == '') {
        ...
        $excel = Url::fromRoute('ek_sales.purchases.excel', ['param' => $param])->toString();
        $build['excel'] = [
    ...
    $taskAlerts = !empty($serials)
      ? \Drupal::service('sales.service')->getTaskAlertsBySerial('purchase', $serials, (int) \Drupal::currentUser()->id())
      : [];
    ...
    foreach ($rows as $r) {
      ...
      if (isset($abook[$r->client])) {
        ...
        $supplier = AddressBookData::geturl($r->client, ['short' => 8]);
      }
  ...
  public function AgingPurchases() {
    ...
    if (isset($_SESSION['coidfilter']['coid'])) {
      ...
      $fields = ['id', 'head', 'allocation', 'serial', 'client', 'status', 'title', 'currency', 'date', 'due',
      ...
      while ($r = $data->fetchObject()) {
        ...
        $client = AddressBookData::geturl($r->client);
        $co = $companies[$r->head];
        ...
        foreach ($groups as $group) {
          if (isset($options[$group])) {
            $build['aging_table_' . $group] = [
              '#type' => 'table',
              '#header' => $header,
              '#rows' => $options[$group],
              '#attributes' => ['id' => 'aging_table_' . $group],
  ...
  public function alertPurchases(Request $request, $id) {
    ...
    if ($data) {
      ...
      $url = Url::fromRoute('ek_sales.purchases.list', [])->toString();
      $items['type'] = 'edit';
      $items['message'] = ['#markup' => $this->t('@document cannot be edited.', ['@document' => $this->t('Alert')])];
  ...
  public function TaskPurchases(Request $request, $id) {
    ...
    $query = Database::getConnection('external_db', 'external_db')
      ->select('ek_sales_purchase', 'p');
    $query->leftJoin('ek_sales_purchase_tasks', 't', 'p.serial=t.serial AND t.uid=' . (int) \Drupal::currentUser()->id());
    $or1 = $query->orConditionGroup();
    $or1->condition('head', $access, 'IN');
    $or1->condition('allocation', $access, 'IN');
    ...
    if ($data) {
      ...
      $items['type'] = 'edit';
      $items['message'] = ['#markup' => $this->t('@document cannot be edited.', ['@document' => $this->t('Task')])];
      $items['description'] = ['#markup' => $this->t('Access denied')];
      $items['link'] = ['#markup' => $this->t('Go to <a href="@url">List</a>.', ['@url' => $url])];
      $build = [
        '#items' => $items,
  ...
  public function ListTaskPurchases() {
    ...
    if (isset($_SESSION['taskfilter']) && $_SESSION['taskfilter']['filter'] == 1) {
      ...
      $build['purchases_tasks_table'] = [
        '#type' => 'table',
        '#header' => $header,
        '#rows' => $data['list'],
        '#attributes' => ['id' => 'purchases_tasks_table'],
        '#empty' => $this->t('No task available'),
        '#attached' => [
          'library' => ['ek_sales/ek_sales_css'],
        ],
      ];
    }
  ...
  public function PrintSharePurchases($id) {
    ...
    if (in_array($data->head, $access) || in_array($data->allocation, $access)) {
      ...
      return [
        ...
        '#theme' => 'iframe',
        '#attached' => [
          'library' => ['ek_sales/ek_sales_print', 'ek_admin/ek_admin_css'],
        ],
      ];
    }
    else {
    ...
    if (in_array($doc_data->head, $access) || in_array($doc_data->allocation, $access)) {
      ...
      if (isset($_SESSION['printfilter']['filter']) && $_SESSION['printfilter']['filter'] == $id) {
        ...
        $build['purchase'] = [
          '#markup' => $document,
          '#attached' => [
            'library' => ['ek_sales/ek_sales_html_documents_css', 'ek_admin/ek_admin_css'],
          ],
        ];
      }
  ...
  public function DeletePurchases(Request $request, $id) {
    ...
    if ($status == '0') {
      ...
      $url = Url::fromRoute('ek_sales.purchases.list', [], [])->toString();
      $items['type'] = 'delete';
      $items['message'] = ['#markup' => $this->t('@document cannot be deleted.', ['@document' => $this->t('Purchase')])];
      $items['description'] = ['#markup' => $opt[$status]];
      $items['link'] = ['#markup' => $this->t('Go to <a href="@url">List</a>.', ['@url' => $url])];
      $build['content'] = [
        '#items' => $items,
