...
class Post extends FormBase {
    ...
    public function buildForm(array $form, FormStateInterface $form_state, $id = null) {
        ...
        $form['del_logistics'] = array(
            '#type' => 'item',
            '#markup' => $this->t('Post data for ref. @p', array('@p' => $data->serial)),
        );

        if (($table == 'delivery' && $data->status == 2) || ($table == 'receiving' && $data->status == 1)) {
            $form['for_id'] = array(
                '#type' => 'hidden',
                '#value' => $id,
            );
            ...
            $form['alert'] = array(
                '#type' => 'item',
                '#markup' => $this->t('Quantities cannot be posted for this order.'),
            );
        }
