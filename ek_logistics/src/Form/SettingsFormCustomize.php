...
/**
 * @file
 * Contains \Drupal\ek_logistics\Form\SettingsFormCustomize.
 */
...
class SettingsFormCustomize extends FormBase {
    ...
    public function __construct() {
        $this->settings = new LogisticsSettings('0');
    }
    ...
    public function getFormId() {
        return 'ek_logistics_form_customize';
    }

    /**
     * {@inheritdoc}
     */
    public function buildForm(array $form, FormStateInterface $form_state, $coids = NULL) {
        ...
        $list['Default'] = ['0|default_delivery_pdf' => $this->t('default delivery')];
        $list['Default'] += ['0|default_receiving_pdf' => $this->t('default receiving')];
        $list['Default'] += ['0|default_returning_pdf' => $this->t('default returning')];
        foreach ($coids as $coid) {
            $templates = 'private://logistics/templates/' . $coid . '/' . $format;
            if (file_exists($templates)) {
                $handle = opendir('private://logistics/templates/' . $coid . '/' . $format);
                while ($file = readdir($handle)) {
                    if ($file != '.' and $file != '..') {
                        if (strpos($file, $format)) {
                            $filename = explode('.', $file);
                            $list['Custom'][$coid . '|' . $file] = $filename[0];
                        }
                    }
                }
            }
        ...
        if ($form_state->get('step') == 2) {
            ...
            $form['body']['col_6'] = [
                ...
                '#min' => 0,
                '#max' => 200,
                '#description' => $this->t('quotation only'),
                '#default_value' => ($s[$tpl[1]]['body']['col_6']) ?
                $s[$tpl[1]]['body']['col_6'] : 15,
    ...
    public function preview(array &$form, FormStateInterface $form_state) {
        ...
        $form['preview'] = [
            ...
            '#context' => [
                'url' => '/ek_logistics/admin/settings-form-preview',
            ],
            '#prefix' => '<div id="preview">',
            '#suffix' => '</div>',
        ];
