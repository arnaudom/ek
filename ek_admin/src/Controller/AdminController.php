...
class AdminController extends ControllerBase {
    ...
    public static function create(ContainerInterface $container) {
        return new static(
                ...
                $container->get('ek_admin.task_notification')
        );
    }
    ...
    public function __construct(
        ...
        $this->taskNotificationService = $task_notification_service;
    }
    ...
    public function run_task_cron($key) {
        ...
        return new Response('', 204);*/
            ...
            $settings = new \Drupal\ek_admin\GlobalSettings(0);

            // =============================================
            // NEW: Use TaskNotificationService for task notifications
            // =============================================
            $stats = $this->taskNotificationService->processAllNotifications($settings);
            
            // Process the mail queue generated by the service
            $mailQueue = $this->taskNotificationService->getMailQueue();
            foreach ($mailQueue as $params) {
                $this->send_mail($params);
            }

            if ($this->moduleHandler->moduleExists('ek_sales')) {
                if ($settings->get('sale_status') == 1) {
                    include_once \Drupal::service('extension.path.resolver')->getPath('module', 'ek_admin') . '/cron/sales_status.inc';
                }
            }

            if ($this->moduleHandler->moduleExists('ek_projects')) {
                if ($settings->get('project_status') == 1) {
                    include_once \Drupal::service('extension.path.resolver')->getPath('module', 'ek_admin') . '/cron/projects_status.inc';
                }
            }

            // Log completion
            $log = 'tasks checking completed. Notifications queued: ' . count($mailQueue);
            \Drupal::logger('ek_admin')->notice($log);
        }
        ...
        if (isset($params['type']) && $params['type'] == 'client') {
            ...
            if ($u) {
                ...
                if ($params['type'] == 'status') {
                    ...
                    $a = \Drupal\user\Entity\User::load($params['assign']);
