...
class TipPluginHText extends TipPluginBase implements ContainerFactoryPluginInterface {
    ...
    public function getOutput() {
        ...
        if ($img = $this->getImage()) {
            $parts = explode(",", $img);
            if (file_exists(\Drupal::service('extension.path.resolver')->getPath('module', $parts[0]) . $parts[1])) {
                $img = \Drupal::service('file_url_generator')->generateAbsoluteString(\Drupal::service('extension.path.resolver')->getPath('module', $parts[0]) . $parts[1]);
