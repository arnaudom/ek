 ...
 */
class TestFilterMailDoc extends FormBase {
...
if($this->moduleHandler->moduleExists('ek_address_book')) {  
      ...
      $form['maildoc']['email'] = array(
        ...
        '#id' => 'edit-email',
        //'#required' => TRUE,
        '#attributes' => array('placeholder' => t('enter email addresses separated by comma (autocomplete enabled).')),
        '#attached' => array(
          'library' => array(
      ...
      $form['maildoc']['message'] = array(
        '#type' => 'textarea',
        '#rows' => 2,
        '#attributes' => array('placeholder' => t('add optional message')),
      );
...
* filter emails
   ...
   $recipients = array();
    ...
    return $form['maildoc']['alert'];
       ...
       if($error != '') {
        ...
        $form['maildoc']['alert']['#prefix'] = "<div id='SendMessage' class='messages messages--error'>";
        $form['maildoc']['alert']['#markup'] = t('Address(es) not valid: @a', array('@a' => $error));
  ...
  $send = mail_attachment($recipients, $file, $message, $options);
      ...
      if($send['error'] == '') {
       ...
       $form['maildoc']['alert']['#markup'] = t('Document sent to @t.', array('@t' => $send['send'])); 
      } else {
        $form['maildoc']['alert']['#prefix'] = "<div id='SendMessage' class='messages messages--error'>";
        $form['maildoc']['alert']['#markup'] = t('Document not sent to @t', array('@t' => $send['error'])); 
    
      
      }
      if($form_state->getValue('source') != 'project_documents') unlink($file);
      return $form['maildoc']['alert'];         
      }
